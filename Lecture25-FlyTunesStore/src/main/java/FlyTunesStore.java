...
public class FlyTunesStore extends ConsoleProgram {
	...
	private int getSelection() {
		println();
		println("Please make a selection (0 to quit):"); println("1. List all songs");
		println("2. List all albums");
		println("3. Add a song");
		println("4. Add an album");
		println("5. List songs on an album"); println("6. Update song price");
	 ...
	 */
	private int findSong(String name, String band) { int index = -1;
	for(int i = 0; i < songs.size(); i++) {
	...
	private static final int LIST_SONGS_ON_ALBUM = 5; private static final int UPDATE_SONG_PRICE = 6;
